...
const Header = () => {
    ...
    const allLinks = [
        ...
        { name: 'Sobre mim', href: '/sobre', showOnDesktop: true },
    ...
    // Array de links sociais com os SVGs completos
    const socialLinks = [
            ...
            <header className="flex items-center justify-between w-full max-w-[1360px] mx-auto backdrop-blur-sm border border-[#E7E7E7] rounded-[20px] h-[60px] pl-5 shadow-sm">
                ...
                <nav className="hidden lg:flex items-center space-x-6">
                    {allLinks
                        ...
                        .map((link) => (
                            <Link key={link.name} to={link.href} className="text-[#5B6270] text-base font-semibold hover:text-[#27527B] transition-colors">
                                {link.name}
                            </Link>
                    ))}
                </nav>
